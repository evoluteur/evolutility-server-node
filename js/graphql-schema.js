...
const gqlField = (f) => ({
  ...
  description: f.label + " - " + f.type + (f.help ? " - " + f.help : ""),
});
...
const gqlOne = (m) => ({
  ...
  resolve(parentValue, args) {
    ...
    const { sql, sqlParams } = crud.SQLgetOne(args.id, moma.getModel(m.id), {
      id: args.id,
    });
    logger.logSQL(sql);
    return db.conn
...
const gqlMany = (m) => ({
  ...
  resolve(parentValue, args) {
    ...
    logger.logSQL(sql);
    return db.conn
      .query(sql, sqlProps.params) //.one(sql)
      .then((data) => {
